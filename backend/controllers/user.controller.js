...
export const register = async (req, res) => {
    try {
        ...
        if (user) {
            return res.status(401).json({
                message: "This email already used to create account ! Try another one",
                success: false
            });
        };
...
export const followOrUnfollow = async (req, res) => {
    try {
        const followKrneWala = req.id; //loggedInUser
        const jiskoFollowKrunga = req.params.id; //Influencer
